import 'dart:async';
import 'package:Foodyy/modules/setting/setting.dart';
import 'package:Foodyy/src/blocs/Bloc.dart';
...
class CityDetailBloc implements Bloc {
  ...
  void fetchPlaces(int? cityId) async {
    final response = await PlaceProvider.getPlaces("$cityId",
    ...
    if (response.json != null && response.json!.isNotEmpty) {
      tmp = List<Place>.generate(
          response.json!.length, (i) => Place.fromJson(response.json![i]));
      places.addAll(tmp);
      _groupData(tmp);
